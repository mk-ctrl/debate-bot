...
import { Users, Crown, LogOut, Send, Mic, Play, Pause, Square, Sun, Moon } from 'lucide-react';

// =================================================================
// --- MOCK DATA (For UI Development without a Backend) ---
// =================================================================
...
const MOCK_ROOM_DATA = {
    ...
    players: [
        { id: 'user-remote-456', name: 'Alice', color: '#db2777', isHost: true },
        { id: 'user-remote-789', name: 'Bob', color: '#059669', isHost: false },
    ],
};
...
// =================================================================
// --- SHARED UI COMPONENTS (Normally in a separate file) ---
// =================================================================
...
export default function DebateRoomPage({ roomCode, playerName, userId, setPage }) {
    ...
    useEffect(() => {
        // Add the current user to the players list when the component mounts
        const playerExists = roomData.players.some(p => p.id === userId);
        if (!playerExists) {
            const newPlayer = {
                id: userId,
                name: playerName || MOCK_PLAYER_NAME,
                color: '#4f46e5',
                isHost: false,
            };
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col md:flex-row p-2 md:p-4 gap-4 transition-colors">
            ...
            <div className="w-full md:w-1/3 lg:w-1/4 bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col">
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">Your ID: {userId}</p>
                    ...
                    {messages.map(msg => {
                        ...
                        return (
                            <div key={msg.id} className={`flex items-start gap-3 my-4 ${isYou ? 'flex-row-reverse' : ''}`}>
                                {!isYou && player && <UserAvatar name={player.name} color={player.color} />}
                                {isYou && <UserAvatar name={playerName} color="#4f46e5" />}
                                <div className={`flex flex-col ${isYou ? 'items-end' : 'items-start'}`}>
                                    ...
                                    <div className={`p-3 rounded-lg max-w-xl ${isYou ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-bl-none'}`}>
                                        {msg.type === 'text' ? <p className="text-base">{msg.text}</p> : <AudioPlayer src={msg.audioURL} duration={msg.duration} />}
                                    </div>
                                </div>
                            </div>
                        );
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    {newMessage ? (
                        ...
                        <PrimaryButton 
                          ...
                          className={`!w-auto px-3.5 ${isRecording ? 'bg-red-600 hover:bg-red-700 animate-pulse' : ''}`}
                        >
